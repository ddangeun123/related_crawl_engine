...
async def search_google(keywords: str):
    results = default_search(keywords=keywords, limit=30, type='Google_test')
    return results

@app.get("/multi_search/google")
async def multi_search(keyword: str):
    queries_list = keyword.split(',')
    print(len(queries_list))
    
    results = crawler.multi_window_search(queries_list)
    return results
...
def default_search(keywords:str, limit:int, type:str, delay:float=0.0):
    ...
    type_map = {
        ...
        'Google_test':crawler.Google_Search_Test
    }
    ...
    for keyword in keywords:
        if current > current_limit:
            crawler.Set_Browser()
            current_limit += limit
        result, succesed = type_map[type](keyword)
